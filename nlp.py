...
class NaturalLanguageProcessing:
    ...
    def generateWordsFrequencyDictionary(periodo, diretorio):
        ...
        dfControle = pd.read_csv(arquivoControle, sep=';', encoding='utf-8', usecols=['full_text_clean'],
                                 iterator=True,
                                 chunksize=chunksize)
    ...
    def createDBText(self, csv_input, csv_output, diretorio):
        usecols = ['created_at', 'id', 'id_screen_name', 'screen_name', 'full_text']
        for user in tqdm.tqdm(csv_input):
            ...
            if not os.path.isfile(diretorio + csv_output):
                df.to_csv(diretorio + csv_output, sep=';', encoding='utf-8')
            else:
                df.to_csv(diretorio + csv_output, sep=';', encoding='utf-8', mode='a', header=False)
